...
| Wallet: deposits, withdrawals, limits, statement | Built, payments mocked |
...
| Screenshot evidence: immutable storage, hashing, duplicate detection, OCR | Built (OCR engine is pluggable; the default is a development stub) |
...
| Live payments (Stripe Connect / PayPal / bank) | **Not built.** The wallet uses a mock provider behind a real payment-intent record — see "Before going live". |
...
98 tests across 8 suites, run against a real PostgreSQL database rather than a
...
- Rake is **10%** of the pool by default, **7%** if either player subscribes to
  Pro. Both are runtime settings, not constants in code.
...
screenshot goes through, with a Hamming distance threshold of 6.
...
- **Responsible play** limits ratchet one way: a player can tighten a deposit or
  ...
  cannot be lifted early by anyone.
...
1. **Payments are mocked.** `payment_intents` records every movement and the
   ledger posting is real, but no processor is called. Wiring Stripe Connect
   means replacing the mock capture with a webhook-confirmed one; the rest of
   the flow — limits, throttles, ledger, reconciliation — does not change.
2. **Email, SMS and KYC document handling are stubs.** The endpoints set the
   ...
   only those handlers.
3. **OCR needs a real engine and a JPEG decoder.** The default engine is a
   development stub, and the built-in image decoder handles PNG only — a PS5
   share export is JPEG, so perceptual hashing currently falls back to content
   hashing for those. Both are single-file changes (`src/ocr/`).
